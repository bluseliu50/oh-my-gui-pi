...
import type { LoadContext } from "oh-my-gui-pi/capability/types";
import { getConfigDirs } from "oh-my-gui-pi/config";
import { getUserPath } from "oh-my-gui-pi/discovery/helpers";
...
describe("PI_CONFIG_DIR", () => {
	...
	test("getConfigDirs respects PI_CONFIG_DIR for user base", () => {
		...
		expect(result[0]).toEqual({ path: expected, source: ".config/omp", level: "user" });
	});
});
