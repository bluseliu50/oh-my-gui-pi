...
import { USER_APP_DIR_NAME } from "@oh-my-pi/pi-utils";
export async function initXdg(): Promise<void> {
	...
	const dirs = [
		path.join(dataHome, USER_APP_DIR_NAME),
		path.join(stateHome, USER_APP_DIR_NAME),
		path.join(cacheHome, USER_APP_DIR_NAME),
	];
	...
	console.log(`are set in your shell profile for ${USER_APP_DIR_NAME} to use them.`);
}
