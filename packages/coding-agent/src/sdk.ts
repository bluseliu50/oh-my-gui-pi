...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		const toolSession: ToolSession = {
			...
			getPendingActions: () => session.getPendingActions(),
			registerPendingAction: action => session.registerPendingAction(action),
			resolvePendingAction: actionId => session.resolvePendingAction(actionId),
